...
def get_avoid_points(avoid_points_file):
    """
    :param avoid_points_file:
    :return: [3242,435,32543,234] list of integer numbers
    """
    text_file = open(avoid_points_file, 'r')
...
def create_hybrid_raincell(dir_path, run_date, run_time, forward, backward, res_mins = '60', model_prefix ='wrf', forecast_source ='wrf0', run_name ='hourly_run'):
    try:
        ...
        avoid_points_file = get_resource_path('extraction/local/avoid_points_250m.txt')
        ...
        with open(config_path) as json_file:
            ...
            if not os.path.isfile(raincell_file_path):
                ...
                avoid_points = get_avoid_points(avoid_points_file)
                print('avoid_points : ', avoid_points)
                #"""
                ...
                if len(validated_obs_station) >= 3 and bool(observed_precipitations):
                    ...
                    if bool(forecast_precipitations):
                        ...
                        for point in points:
                            if point[0] in avoid_points:
                                fcst_point_thess_idx.append(None)
                            else:
