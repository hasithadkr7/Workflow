...
def download_netcdf(initial_path_prefix, download_location, net_cdf_file_name, key_file, bucket_name):
    try:
        ...
        for blob in blobs:
            if fnmatch.fnmatch(blob.name, "*" + net_cdf_file_name):
                print(blob.name)
                directory = os.path.join(download_location, net_cdf_file_name)
                if not os.path.exists(download_location):
                    os.makedirs(download_location)
                    print('download_netcdf|download_location: ', download_location)
                try:
                    print('download_to_filename|directory : ', directory)
                    #blob.download_to_filename(directory)
                    blob.download_to_filename(net_cdf_file_name)
                except Exception as ex:
                    print('download_netcdf|download_to_filename|Exception : ', str(ex))
...
def extract_kelani_basin_rainfall_flo2d_with_obs(run_time, nc_f, adapter, obs_stations, output_dir, start_ts_lk,
    ...
    if kelani_lower_basin_points is None:
        kelani_lower_basin_points = get_resource_path('extraction/local/kelani_basin_points_250m.txt')
...
def create_hybrid_raincell(dir_path, run_date, run_time, forward=3, backward=2):
    try:
        ...
        with open(config_path) as json_file:
            ...
            key_file = os.path.join(os.getcwd(), 'raincelldat', 'uwcc-admin.json')
            ...
            try:
                net_cdf_file_path = os.path.join(download_location, net_cdf_file_name)
                print("net_cdf_file_path : ", net_cdf_file_path)
                if not os.path.isfile(net_cdf_file_path):
                    download_netcdf(initial_path_prefix, download_location, net_cdf_file_name, key_file, bucket_name)
