        ...
        if not os.path.isfile(raincell_file_path):
            ...
            with open(config_path) as json_file:
                ...
                if db_config is not None:
                    print(db_config)
                    db_adapter = CurwSimAdapter(db_config['user'], db_config['password'], db_config['host'],
                                                db_config['db'])
                ...
                if db_adapter is not None or flo2d_model_config:
                    ...
                    for item in id_date_list:
                        tms_df = get_cell_timeseries(db_adapter, item['hash_id'],
                                                     item['obs_end'].strftime('%Y-%m-%d %H:%M:%S'), timeseries_start,
                                                     timeseries_end, res_mins)
                        if tms_df is not None:
...
def prepare_flo2d_250_MME_raincell_5_min_step(start_time, end_time, raincell_file_path):
    ...
    write_to_file(raincell_file_path,
                  '{} {} {} {}\n'.format(5, length, start_time.strftime(DATE_TIME_FORMAT),
...
def create_sim_hybrid_raincell(dir_path, run_date, run_time, forward, backward,
    ...
    if not os.path.isfile(raincell_file_path):
        if '250m' == flo2d_model:
            create_sim_hybrid_raincells(dir_path, run_date, run_time, forward, backward,
                                        res_mins=res_mins, flo2d_model='flo2d_250',
                                        calc_method=calc_method)
        elif '150m' == flo2d_model:
            create_sim_hybrid_raincells(dir_path, run_date, run_time, forward, backward,
                                        res_mins=res_mins, flo2d_model='flo2d_150',
                                        calc_method=calc_method)
