...
def create_outflow(dir_path, ts_start, ts_end, model):
    ...
    if not os.path.isfile(outflow_file_path):
        if '250m' == model:
            prepare_outflow_250(outflow_file_path, start=ts_start, end=ts_end, tide_id=tide_id)
        elif '150m' == model:
            prepare_outflow_150(outflow_file_path, start=ts_start, end=ts_end, tide_id=tide_id)
    ...
    try:
        ...
        if output_dir is not None and file_name is not None:
            ...
            outflow_file_path = os.path.join(r"D:\outflow",
                                             '{}_{}_{}_{}.DAT'.format(file_name, flo2d_model, start_time,
                                                                      end_time).replace(' ', '_').replace(':', '-'))
