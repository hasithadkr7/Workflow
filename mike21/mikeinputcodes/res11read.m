function [vals, outInfos, TimeStampsC] = res11read(infile, extractPoints,chainagetol)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Start datetime detection
d=res11File.FileInfo.TimeAxis.StartDateTime.Day;
mo=res11File.FileInfo.TimeAxis.StartDateTime.Month;
y=res11File.FileInfo.TimeAxis.StartDateTime.Year;
h=res11File.FileInfo.TimeAxis.StartDateTime.Hour;
mi=res11File.FileInfo.TimeAxis.StartDateTime.Minute;
s=res11File.FileInfo.TimeAxis.StartDateTime.Second;
startdatetimeS=sprintf('%d-%d-%d %d:%d:%d',y,mo,d,h,mi,s);
startdatetimeT=datetime(startdatetimeS,'InputFormat','yyyy-MM-dd HH:mm:ss');
TimeStampsT = datetime.empty();
TimeStampsC = cell(res11File.FileInfo.TimeAxis.NumberOfTimeSteps,1);
TimeStampsT(1) = startdatetimeT;
for d = 2:res11File.FileInfo.TimeAxis.NumberOfTimeSteps
    TimeStampsT(d,1) = TimeStampsT(d-1,1) + minutes(15);
end
TimeStampsT.Format = 'default';
for d = 1:res11File.FileInfo.TimeAxis.NumberOfTimeSteps
    TimeStampsC(d,1) = cellstr(TimeStampsT(d,1));
end
...
%d1 = vals(1,1);
%datestr(d1 + 695422, 'HH:MM:SS')
%datetime(d1,'ConvertFrom','excel1904') % or 'excel1904'
